...
private:
    std::vector<IIR::Library_Declaration*> Libraries_List;
    std::vector<std::filesystem::path> paths;
    State state;

    std::string Library_To_File_Name(IIR::Library_Declaration* Library);
    bool Search_Library_In_Path(IIR::Library_Declaration* Library);
public:
...
//    Location_Type Command_Line_Location;
    ...
    IIR::Library_Declaration* Std_Library = nullptr;
    ...
    IIR::Library_Declaration* Work_Library;
    ...
    IIR::Base* Obsoleted_Design_Units = nullptr;
    ...
    IIR::Design_File* Load_File(std::filesystem::path file);
    ...
    void Load_Design_Unit(IIR::Design_Unit* Design_Unit, IIR::Base* Loc);
    ...
    void Load_Parse_Design_Unit(IIR::Design_Unit* Design_Unit, IIR::Base* Loc);

    // Remove the same file as DESIGN_FILE from work library and all of its units.
    void Purge_Design_File(IIR::Design_File Design_File);

    // Just return the design_unit for NAME, or NULL if not found.
    IIR::Design_Unit* Find_Primary_Unit(IIR::Library_Declaration* Library, std::string Name);
    ...
    IIR::Design_Unit* Load_Primary_Unit(IIR::Library_Declaration* Library, std::string Name, IIR::Base* Loc);
    ...
    IIR::Design_Unit* Find_Secondary_Unit(IIR::Design_Unit* Primary, std::string Name);

    // Load an secondary unit of primary unit PRIMARY and analyse it.
    // NAME must be set only for an architecture.
    IIR::Design_Unit* Load_Secondary_Unit(IIR::Design_Unit* Primary, std::string Name, IIR::Base* Loc);

    // Analyze UNIT.
    void Finish_Compilation(IIR::Design_Unit* Unit, bool Main = false);

    // Get or create a library from an identifier.
    // LOC is used only to report errors.
    IIR::Library_Declaration* Get_Library(std::string Ident, Location_Type Loc);
    ...
    void Add_Design_Unit_Into_Library(IIR::Design_Unit* Unit, bool Keep_Obsolete = false);
    ...
    void Add_Design_File_Into_Library(IIR::Design_File* File);
    ...
    IIR::Design_Unit* Find_Design_Unit(IIR::Selected_Name* Unit);
    IIR::Design_Unit* Find_Design_Unit(IIR::Entity_Aspect_Entity* Unit);
    ...
    IIR::Design_Unit* Find_Entity_For_Component(std::string Name);

    // Get the chain of libraries.  Can be used only to read (it musn't be modified).
    std::vector<IIR::Library_Declaration*> Get_Libraries_List();

    void Purge_Design_File(IIR::Design_File* Design_File);

    void Save_Library(IIR::Library_Declaration* Library);

    bool Load_Library(IIR::Library_Declaration* Library);
};
