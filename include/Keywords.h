...
//const std::string Name_Union = "union";
    ...
    const std::string KeywordList[Name_Last_PSL+1] = {
            Name_Mod,
            Name_Rem,
...
// Create operators.
            ...
            Name_Op_Match_Less,
            Name_Op_Match_Less_Equal,
            Name_Op_Match_Greater,
            Name_Op_Match_Greater_Equal,
...
//  Create standard.
            ...
            Name_Boolean_Vector,
            Name_To_Bstring,
            Name_To_Binary_String,
            Name_To_Ostring,
            Name_To_Octal_String,
            Name_To_Hstring,
            Name_To_Hex_String,
            Name_Integer_Vector,
            Name_Real_Vector,
...
// Create misc.
            ...
            Name_Minimum,
            Name_Maximum,
            Name_Untruncated_Text_Read,
            Name_Get_Resolution_Limit,
            Name_Control_Simulation,

            Name_Ieee,
            Name_Std_Logic_1164,
            Name_Std_Ulogic,
            Name_Std_Ulogic_Vector,
            Name_Std_Logic,
            Name_Std_Logic_Vector,
            Name_Rising_Edge,
            Name_Falling_Edge,
            Name_VITAL_Timing,
            Name_VITAL_Level0,
            ...
            Name_Unsigned,
            Name_Signed,
            Name_Unresolved_Unsigned,
            Name_Unresolved_Signed,
            Name_Std_Logic_Arith,
            Name_Std_Logic_Signed,
            Name_Std_Logic_Textio,
...
//  PSL keywords
    ...
    inline bool isKeyword(const std::string &id) {
        auto i = KeywordTable.find(id);
        return (i != KeywordTable.end() /*&& i->second >= Name_First_Keyword*/ && i->second <= Name_Last_AMS_Keyword);
    }

    inline bool isAMSReservedWord(const std::string &id) {
        auto i = KeywordTable.find(id);
        return (i != KeywordTable.end() && i->second >= Name_First_AMS_Keyword && i->second <= Name_Last_AMS_Keyword);
    }

    inline bool isVHDL08ReservedWord(const std::string &id) {
        auto i = KeywordTable.find(id);
        return (i != KeywordTable.end() && i->second >= (Name_First_Keyword + 98)
                && i->second <= (Name_First_Keyword + 99));
    }

    inline bool isVHDL00ReservedWord(const std::string &id) {
        auto i = KeywordTable.find(id);
        return (i != KeywordTable.cend() && i->second == (Name_First_Keyword + 97));
    }

    inline bool isVHDL93ReservedWord(const std::string &id) {
        auto i = KeywordTable.find(id);
        return (i != KeywordTable.end() && i->second >= (Name_First_Keyword + 81) &&
                i->second <= (Name_First_Keyword + 96));
    }

    inline bool isVHDL87ReservedWord(const std::string &id) {
        auto i = KeywordTable.find(id);
        return (i != KeywordTable.end() /*&& i->second >= Name_First_Keyword*/ &&
                i->second <= (Name_First_Keyword + 80));
    }
