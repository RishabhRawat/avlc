...
class ClassDesc:
    ...
    def print_class(self):
        if self.alias:
            print 'using ' + self.name + ' = ' + self.alias + ';\n'
        else:
            class_inh = ['public ' + b for b in self.base]
            inh_list = ', '.join(class_inh)
            print 'struct ' + self.name + ': ' + inh_list + ' {'
            for field, ftype in self.members.items():
                print '    ' + ftype + ' ' + field + ';'
            print '};\n'

    def add_base(self, base):
        if self.base == {'virtual Iir'}:
            self.base = [base.name]
        else:
            self.base.append(base.name)
        base.derived.add(self)

    def merge_class(self, other):
        self.base.union(other.base)
        self.members.update(other.members)
        other.alias = self.name
...
def convertType(name):
    return higher_type_map.get(name, name)


def mergeClassList(classList):
    i = 0
    for clss in classList:
        if clss.base == {'virtual Iir'} and clss.derived and clss.alias == '':
            for cl in classList[classList.index(clss) + 1:]:
                if clss.derived == cl.derived:
                    clss.merge_class(cl)
...
if args.action == 'disp-formats':
    ...
    while True:
        ...
        if l == '      --  FIELDS_TYPE':
            ...
            print l
elif args.action == 'avlc-iir':
    # for k in kinds:
    #     v = nodes[k]
    #     print 'struct Iir_' + k + ': public Iir {'
    #     flds = [fk for fk, fv in v.fields.items() if fv]
    #     for fk in sorted(flds):
    #         print '  ' + type_map[fk] + ' ' + v.fields[fk].name + ';'
    #     print '};\n'


    type_map = {}
    for f in funcs:
        type_map[f.name] = convertType(f.rtype)
        ...
        if len(choices) == 0:
            ...
            for ch in choices:
                ...
                while i < len(classList):
                    if 'Iir_' + ch == classList[i].name:
                        if firsttime:
                            base = ClassDesc(f.name + '_Abs')
                            if f.rtype in type_map:
                                base.members[f.name] = convertType(type_map[f.rtype])
                            elif f.rtype in types:
                                base.members[f.name] = f.rtype
                            else:
                                raise AssertionError('sdss')
                            base.members[f.name] = classList[i].members[f.name]
                            classList.insert(i, base)
                            i = i + 1
                            firsttime = False
                        del classList[i].members[f.name]
                        classList[i].add_base(base)
                    i = i + 1
    mergeClassList(classList)
    for cl in classList:
        cl.print_class()
